  ...
  return (
    <div className="min-h-screen bg-[#FAF8F5] pb-28 max-w-md mx-auto select-none">
      <header className="sticky top-0 z-30 bg-[#FAF8F5]/90 backdrop-blur-md px-4 py-3 flex items-center gap-3 border-b border-[#E8E4DA]/50">
        ...
        <h1 className="text-base font-bold text-[#11241C]">
          Request Service
        </h1>
      </header>
      ...
      <form onSubmit={handleSubmit} className="p-5 space-y-5">
        {/* Worker Mini Card */}
        <div className="bg-white rounded-3xl p-4 border border-[#E8E4DA] shadow-xs flex items-center gap-3.5">
          <img
            src={worker.avatarUrl}
            alt={worker.name}
            className="w-12 h-12 rounded-2xl object-cover border border-[#E8E4DA]"
          />
          <div>
            <h3 className="font-extrabold text-sm text-[#11241C]">{worker.name}</h3>
            <p className="text-xs font-semibold text-[#063B2C]">{worker.profession}</p>
            <p className="text-[11px] text-[#55685F] mt-0.5">{worker.startingPrice}</p>
          </div>
        </div>
        ...
        <div className="space-y-2">
          ...
          <select
            ...
            className="w-full bg-white border border-[#D2CEBE] rounded-2xl p-3.5 text-xs font-bold text-[#11241C] focus:outline-none"
          >
        ...
        <div className="space-y-2">
          ...
          <textarea
            ...
            className="w-full bg-white border border-[#D2CEBE] rounded-2xl p-3.5 text-xs font-semibold text-[#11241C] focus:outline-none focus:border-[#063B2C] resize-none"
          ></textarea>
        </div>
        ...
        <div className="space-y-2">
          ...
          <div className="flex items-center gap-2 bg-white border border-[#D2CEBE] rounded-2xl p-3">
            <MapPin className="w-4 h-4 text-[#063B2C] shrink-0" />
            <input
              type="text"
              value={address}
              onChange={(e) => setAddress(e.target.value)}
              className="w-full text-xs font-bold text-[#11241C] focus:outline-none"
            />
          </div>
        </div>
        ...
        <button
          ...
          className="w-full py-4 rounded-2xl bg-[#063B2C] text-white font-bold text-sm shadow-md hover:bg-[#084D3A] active:scale-98 transition-all cursor-pointer disabled:opacity-50"
        >
