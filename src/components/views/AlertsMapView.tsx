...
export const AlertsMapView: React.FC = () => {
  ...
  return (
    <div className="w-full relative min-h-screen bg-[#FAF8F5] dark:bg-[#020617] text-[#0F172A] dark:text-[#E2E8F0] pb-28 select-none transition-colors duration-200">
      {/* Sticky Header */}
      <header className="w-full sticky top-0 z-40 bg-[#FAF8F5]/95 dark:bg-[#020617]/95 backdrop-blur-md border-b border-[#E8E4DA] dark:border-white/10">
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-1.5">
            ...
            <button
              onClick={toggleTheme}
              className="w-8 h-8 rounded-xl bg-white dark:bg-[#1E293B] border border-[#E8E4DA] dark:border-white/10 flex items-center justify-center text-slate-500 dark:text-slate-400 shadow-xs hover:bg-[#F2EFE9] dark:hover:bg-white/5 transition-colors cursor-pointer"
              title="Toggle theme"
            >
              ...
              <button
                ...
                className={`px-3 py-1.5 rounded-full text-xs font-bold whitespace-nowrap transition-all cursor-pointer ${
                  isActive
                    ...
                    : 'bg-white dark:bg-[#1E293B] text-slate-500 dark:text-slate-400 border border-[#E8E4DA] dark:border-white/10 hover:bg-[#FAF8F5] dark:hover:bg-white/5'
                }`}
              >
                {tab.label}
              </button>
        ...
        {(activeFilter === 'ALL' || activeFilter === 'NOTICES') && (
          <div className="space-y-3 pt-2">
            ...
            {localAlerts.length === 0 ? (
              <div className="p-6 rounded-3xl bg-white dark:bg-[#1E293B] border border-[#E8E4DA] dark:border-white/10 text-center space-y-2">
              ...
              <div className="space-y-2.5">
                {localAlerts.map((alert) => (
                  <div
                    ...
                    className="p-4 rounded-3xl bg-white dark:bg-[#1E293B] border border-[#E8E4DA] dark:border-white/10 shadow-xs space-y-2.5 cursor-pointer hover:border-[#007AFF] dark:hover:border-[#60A5FA] transition-all"
                  >
                    <div className="flex items-start justify-between gap-2">
                      <div className="space-y-0.5">
                        ...
                        <h3 className="text-xs font-extrabold text-[#0F172A] dark:text-white mt-1">
                          {alert.title}
                        </h3>
                      </div>
                    ...
                    <div className="flex items-center justify-between text-[11px] font-bold text-slate-500 dark:text-slate-400 pt-1 border-t border-[#F0ECE1] dark:border-white/5">
      ...
      {selectedNotice && (
        <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-black/60 backdrop-blur-xs p-4">
          <div className="bg-white dark:bg-[#1E293B] rounded-3xl p-5 w-full max-w-sm shadow-2xl border border-[#E8E4DA] dark:border-white/10 space-y-4">
            <div className="flex items-start justify-between">
              <div>
                ...
                <h3 className="text-sm font-extrabold text-[#0F172A] dark:text-white mt-1">
                  {selectedNotice.title}
                </h3>
              </div>
              <button
                onClick={() => setSelectedNotice(null)}
                className="w-7 h-7 rounded-full flex items-center justify-center text-slate-500 dark:text-slate-400 hover:bg-black/5 dark:hover:bg-white/5"
              >
                <X className="w-4 h-4" />
              </button>
            </div>
            ...
            <div className="p-3 rounded-2xl bg-[#FAF8F5] dark:bg-[#020617] border border-[#E8E4DA] dark:border-white/10 space-y-1">
              <p className="text-[11px] font-bold text-[#0F172A] dark:text-white flex items-center gap-1">
