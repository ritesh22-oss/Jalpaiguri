        ...
        <div className="space-y-3 pt-1">
          {filteredRentals.length === 0 ? (
            <div className="text-center py-12 bg-white rounded-3xl p-6 border border-[#E8E4DA]">
              <Building className="w-12 h-12 text-[#8C9B93] mx-auto mb-3 opacity-60" />
              <h3 className="font-bold text-base text-[#11241C]">No listings found</h3>
              ...
              >
                <div className="flex items-start justify-between gap-2">
                  <div className="flex-1">
                    <span className="text-[10px] font-bold uppercase tracking-wider bg-[#E6F4EA] text-[#063B2C] px-2 py-0.5 rounded-md inline-block mb-1">
                      {p.type}
                    </span>
                    <h3 className="font-extrabold text-sm text-[#11241C] leading-snug">{p.title}</h3>
                  </div>
                  <span className="font-extrabold text-sm text-[#063B2C] bg-[#E6F4EA] px-2.5 py-1 rounded-xl shrink-0">
                    {p.rent}
                  </span>
                </div>
                ...
                {p.description && (
                  <p className="text-xs text-[#73827B] line-clamp-2">{p.description}</p>
                )}

                <div className="pt-2 border-t border-[#F0ECE1] flex items-center justify-between">
                  <span className="text-xs font-bold text-[#55685F]">Deposit: {p.deposit}</span>
                  <button
                    onClick={() => window.location.href = `tel:${p.contact.replace(/\s+/g, '')}`}
                    className="px-4 py-2 rounded-xl bg-[#063B2C] hover:bg-[#084D3A] text-white text-xs font-bold flex items-center gap-1.5 active:scale-95 transition-all cursor-pointer"
                  >
        ...
        <div className="fixed inset-0 z-50 bg-black/50 backdrop-blur-xs flex items-end sm:items-center justify-center p-4">
          <div className="bg-white rounded-3xl max-w-md w-full p-5 space-y-4 shadow-xl max-h-[90vh] overflow-y-auto animate-in slide-in-from-bottom-6">
            <div className="flex items-center justify-between border-b border-[#E8E4DA] pb-3">
              <h3 className="font-extrabold text-base text-[#11241C]">List Rental Property</h3>
              <button
            ...
            <form onSubmit={handlePostProperty} className="space-y-3.5">
              <div>
                <label className="block text-xs font-bold text-[#11241C] uppercase mb-1">Title *</label>
                <input
                  type="text"
                  required
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-xs font-bold text-[#11241C] uppercase mb-1">Type</label>
                  <select
                    value={type}
                ...
                <div>
                  <label className="block text-xs font-bold text-[#11241C] uppercase mb-1">Monthly Rent *</label>
                  <input
                    type="text"
                    required
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-xs font-bold text-[#11241C] uppercase mb-1">Security Deposit</label>
                  <input
                    type="text"
              ...
              <div>
                <label className="block text-xs font-bold text-[#11241C] uppercase mb-1">Area / Locality</label>
                <input
                  type="text"
              ...
              <div>
                <label className="block text-xs font-bold text-[#11241C] uppercase mb-1">Description</label>
                <textarea
                  rows={2}
