...
export const AuthView: React.FC = () => {
  ...
  const [signupGender, setSignupGender] = useState<'Male' | 'Female' | 'Other' | 'Prefer not to say'>('Male');
  ...
  const handleGoogleSignIn = async (selectedEmail?: string, selectedName?: string) => {
    ...
    setErrorMsg('');
    try {
      const emailToUse = selectedEmail || 'riteshganguly0911@gmail.com';
      const nameToUse = selectedName || (emailToUse.toLowerCase().includes('ritesh') ? 'Ritesh Ganguly' : 'Citizen');
        ...
        <div className="flex-1 flex flex-col justify-between py-2 animate-in fade-in duration-300">
          ...
          <div className="space-y-3.5 pb-2">
            ...
            <button
              ...
              disabled={loading}
              className="w-full py-3.5 px-4 rounded-2xl bg-white hover:bg-[#F8FAFC] border-2 border-[#CBD5E1] active:scale-98 text-[#1E293B] font-bold text-sm flex items-center justify-center gap-3 shadow-xs hover:border-[#94A3B8] transition-all cursor-pointer"
            >
        ...
        <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-xs flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl max-w-sm w-full p-6 shadow-2xl space-y-4 animate-in zoom-in-95">
            ...
            <p className="text-xs text-[#64748B]">
              Choose a Google account to continue to <strong>Jalpaiguri Connect</strong>:
            </p>

            {/* Quick One-Tap Account Options */}
            <div className="space-y-2">
              <button
                type="button"
                onClick={() => handleGoogleSignIn('riteshganguly0911@gmail.com', 'Ritesh Ganguly')}
                className="w-full text-left p-3 rounded-2xl border border-[#E2E8F0] hover:border-[#2563EB] hover:bg-blue-50/50 transition-all flex items-center gap-3 cursor-pointer group"
              >
                ...
                <div className="flex-1 min-w-0">
                  <div className="text-xs font-bold text-[#11241C] group-hover:text-[#2563EB] truncate">
                    Ritesh Ganguly
                  </div>
                  <div className="text-[11px] text-[#64748B] truncate">
                    riteshganguly0911@gmail.com
                  </div>
                </div>
                <ArrowRight className="w-4 h-4 text-[#94A3B8] group-hover:text-[#2563EB]" />
              </button>

              <button
                type="button"
                onClick={() => handleGoogleSignIn('citizen.jalpaiguri@gmail.com', 'Jalpaiguri Resident')}
                className="w-full text-left p-3 rounded-2xl border border-[#E2E8F0] hover:border-[#2563EB] hover:bg-blue-50/50 transition-all flex items-center gap-3 cursor-pointer group"
              >
            ...
            {/* Custom Google account input divider */}
            <div className="relative flex items-center justify-center pt-1 pb-1">
              <div className="border-t border-[#E2E8F0] w-full"></div>
              <span className="bg-white px-2 text-[10px] uppercase font-bold text-[#94A3B8] absolute">
            ...
            <div className="space-y-3">
              <div>
                <input
                  ...
                  className="w-full border border-[#CBD5E1] rounded-xl px-3.5 py-2.5 text-xs font-semibold focus:border-[#2563EB] focus:outline-none"
                />
              </div>
              ...
              <button
                type="button"
                onClick={() => {
                  const emailToUse = customGoogleEmail.trim() || 'riteshganguly0911@gmail.com';
                  handleGoogleSignIn(emailToUse, signupName.trim() || undefined);
                }}
                className="w-full py-2.5 bg-[#2563EB] hover:bg-[#1D4ED8] text-white text-xs font-bold rounded-xl transition-all shadow-xs cursor-pointer flex items-center justify-center gap-2"
              >
